...
# variable FE
FOLDER_SRC_CODE="/var/lib/jenkins/workspace/epic-dev/src"

FOLDER_FE_HOME="EPIC.Home"
FOLDER_FE_CORE="EPIC.Core"
FOLDER_FE_BOND="EPIC.Ebond"
FOLDER_FE_INVEST="EPIC.Invest"
FOLDER_FE_USER="EPIC.Euser"

# set permission
cd /var/lib/jenkins/workspace/epic-dev/.git
sudo chown -R jenkins:jenkins *

# DEPLOY BACK END
cd /var/lib/jenkins/workspace/epic-dev/src
...
export OCELOT_ENVIRONMENT=DevTest
...
chown -R jenkins:jenkins "$FOLDER_PUBLISH_API_GATEWAY"
dotnet publish -c Release -o "$FOLDER_PUBLISH_API_GATEWAY"
nohup dotnet "$FOLDER_PUBLISH_API_GATEWAY/EPIC.APIGateway.dll" > "$FOLDER_PUBLISH_API_GATEWAY/dotnetcore.log" &

# identity server
echo  === Publish EPIC.IdentityServer ===

cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_IDENTITY_SERVER"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_CORE_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_BOND_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_IMAGE_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_INVEST_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_ROCKETCHAT_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_PAYMENT_SERVICE"
...
cd ../..
chown -R jenkins:jenkins "$FOLDER_PUBLISH_SHARED_SERVICE"
