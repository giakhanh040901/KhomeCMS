...
@Component({
  selector: 'app-create-or-update-deposit-provider',
  templateUrl: './create-or-update-deposit-provider.component.html',
  styleUrls: ['./create-or-update-deposit-provider.component.scss']
})
export class CreateOrUpdateDepositProviderComponent extends AppComponentBase {

  constructor(
    injector: Injector,
    public bsModalRef: BsModalRef,
    private _datePickerLocale: BsLocaleService,
    private _depositProviderService: DepositProviderServiceProxy,
  ...
  depositProviderFieldCurrencyDisplays = {
    'capital': null,
  }

  depositProviderFieldTypeDates = ['foundationDate','licenseDate', 'repIdDate'];
  depositProviderDateDisplays = {
    'foundationDate': null,
    'licenseDate': null,
    'repIdDate' : null
  }
  ...
  // inputElements = ['phone', 'mobile', 'email'];
  ...
  ngOnInit(): void {
    ...
    if(this.depositProvider.depositProviderId) {
      
      this._depositProviderService.get(this.depositProvider.depositProviderId).subscribe((res) => {
        this.depositProvider = res.data;
        //format date output
        this.formatDateOutput(this.depositProviderFieldTypeDates, this.depositProvider, this.depositProviderDateDisplays);
        this.formatCurrencyOutput(this.depositProviderFieldCurrencyDisplays, this.depositProvider );
  ...
  changeDatePicker(field, value) {
    ...
    if(value == 'Invalid Date') {
      ...
      this.depositProvider[field] = value;
    }
  }
  changeCurrency(field, value) {
    this.depositProviderFieldCurrencyDisplays[field] = this.formatCurrency(value);
 }

  changeDate() {
    console.log({ datepicker: this.depositProvider.datepicker });
  }
  ...
  callTriggerFieldError(response) {
    ...
    if(this.depositProvider[keyFirstNameError] !== undefined) {
  ...
  save(): void {
    this.saving = true;
    // Fomat date Input
    this.formatDateInput(this.depositProviderFieldTypeDates, this.depositProvider);
    this.formatCurrencyInput(this.depositProviderFieldCurrencyDisplays, this.depositProvider);
    const data = { ...this.depositProvider };
    if (!this.depositProvider.depositProviderId) {
      this._depositProviderService.create(data).subscribe(
          (response) => {
      ...
      this._depositProviderService.update(data, this.depositProvider.depositProviderId).subscribe((response) => {
