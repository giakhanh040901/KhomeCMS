...
@Component({
    ...
    <div class="card-footer" [hidden]="isCompleted">
        <button type="button" class="btn btn-sm btn-secondary float-left" (click)="previous()" [hidden]="!hasPrevStep || !activeStep.showPrev">{{ textPrev ?? 'Quay lại'}}</button>
        <button type="button" class="btn btn-sm btn-secondary float-right" (click)="next()" [disabled]="!activeStep.isValid" [hidden]="!hasNextStep || !activeStep.showNext">{{ textNext ?? 'Tiếp theo' }}</button>
        <button type="button" class="btn btn-sm btn-secondary float-right" id="button-complete" (click)="complete()" [disabled]="!activeStep.isValid" [hidden]="hasNextStep">{{ textComplete ?? 'Hoàn thành' }}</button>
    </div>
  </div>`
    ,
    styles: [
