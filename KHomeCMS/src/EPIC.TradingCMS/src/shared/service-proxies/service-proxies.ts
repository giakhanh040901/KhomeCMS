...
export class TokenAuthServiceProxy extends ServiceProxyBase {
  ...
  authenticate(
    ...
    .set('grant_type', AppConsts.grantType)
    ...
    if (status === 200) {
      ...
      return blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          const titleMessage = _responseTextPare ? _responseTextPare.message : 'Tài khoản không tồn tại hoặc kết nối mạng bị gián đoạn!';
          abp.notify.error(titleMessage, '', { timer: 3000, position: 'top' });
...
export class UserServiceProxy extends ServiceProxyBase {
  ...
  update(body: any, userId: number): Observable<any> {
    let url_ = "/api/users/update-user/" + userId;
    url_ = url_.replace(/[?&]$/, "");
