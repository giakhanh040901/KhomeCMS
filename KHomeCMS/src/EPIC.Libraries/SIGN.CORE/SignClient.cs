...
#pragma warning restore IDE0044 // Add readonly modifier
        ...
        public SignClient(string server,string apiId, string secret, string service, string region)
        {
            //this.endpointUri = string.Format("https://{0}-{1}.{2}", service, region, host);
            this.endpointUri = server;
        ...
        protected override WebRequest GetWebRequest(Uri address)
        {
            ...
            var headers = new Dictionary<string, string>
            {
                    ...
                    {"content-length", Encoding.UTF8.GetBytes(this.prebuf).Length.ToString()},
        ...
        public new string UploadString(string address, string method, String data)
        {
            ...
            try
            {
                ...
                var byteArray = Encoding.ASCII.GetBytes(sstring);
                return DecodeFromUtf8(sstring);
            }
            catch (WebException ex)
            {
                //var test = ex.InnerException.InnerException.Message;
                throw ex;
            }
            finally
            {
