...
export class MediaComponent extends CrudComponentBase implements OnInit {
  ...
  genListAction(data = []) {
    this.listAction = data.map(item => {
      ...
      if (item.status != this.MediaConst.status.DELETED && this.isGranted([this.PermissionGarnerConst.GarnerHinhAnh_Sua])) {
        actions.push({
          data: item,
          label: 'Sửa',
          icon: 'pi pi-pencil',
      ...
      if (item.status == this.MediaConst.status.DELETED && this.isGranted([this.PermissionGarnerConst.GarnerHinhAnh_ChiTiet])) {
        actions.push({
          data: item,
          label: 'Xem chi tiết',
          icon: 'pi pi-pencil',
      ...
      if (item.status != this.MediaConst.status.DELETED && this.isGranted([this.PermissionGarnerConst.GarnerHinhAnh_Xoa])) {
        actions.push({
          data: item,
          label: 'Xoá',
          icon: 'pi pi-trash',
      ...
      if ((item.status == this.MediaConst.status.PENDING || item.status == this.MediaConst.status.DRAFT) && this.isGranted([this.PermissionGarnerConst.GarnerHinhAnh_DuyetDang])) {
        actions.push({
          data: item,
          label: 'Duyệt đăng',
          icon: 'pi pi-check',
      ...
      if (item.status == this.MediaConst.status.ACTIVE && this.isGranted([this.PermissionGarnerConst.GarnerHinhAnh_DuyetDang])) {
        actions.push({
          data: item,
          label: 'Bỏ duyệt đăng',
          icon: 'pi pi-times',
  ...
  setPage(pageInfo?: any) {
    ...
    this.broadcastService.getAllMedia(this.page, this.status, 'garner', this.position).subscribe((res) => {
