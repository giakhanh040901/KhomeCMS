...
@Component({
  selector: 'app-product-legal-records',
  templateUrl: './product-legal-records.component.html',
  styleUrls: ['./product-legal-records.component.scss']
})
export class ProductLegalRecordsComponent extends CrudComponentBase {
    ...
    productFile: any = {
        ...
        'url': null,
        'productId': 0,
        'documentType': ProductFileConst.HO_SO_PHAP_lY,
    };
    ...
    genListAction(data = []) {
		this.listAction = data.map(orderItem => {
			const actions = [];
        if (this.isGranted([this.PermissionGarnerConst.GarnerSPDT_HSPL_DownloadFile])) {
            actions.push(
            {
                data: orderItem,
                label: 'Tải file',
        ...
        if (this.utils.isPdfFile(orderItem.url) && this.isGranted([this.PermissionGarnerConst.GarnerSPDT_HSPL_Preview])) {
            actions.push(
            {
                data: orderItem,
                label: 'Xem file',
        ...
        if (this.isGranted([this.PermissionGarnerConst.GarnerSPDT_HSPL_CapNhat])) {
            actions.push(
            {
                data: orderItem,
                label: 'Sửa',
        ...
        if (this.isGranted([this.PermissionGarnerConst.GarnerSPDT_HSPL_DeleteFile])) {
			    actions.push({
                    data: orderItem,
                    label: 'Xóa',
                    icon: 'pi pi-trash',
    ...
    create() {
        ...
        this.modalDialog = true;
        this.headerTitle = 'Thêm mới hồ sơ pháp lý';
    }

    confirmDelete() {
        this.deleteItemDialog = false;
        this._contractTemplateService.delete(this.productFile.id).subscribe(
            (response) => {
            ...
            () => {
                this.messageService.add({
                    severity: "error",
                    summary: "",
                    detail: `Không xóa được hồ sơ ${this.productFile.displayName}`,
                    life: 3000,
                });
            }
        );
    ...
    save() {
        ...
        if (this.productFile.id) {
            ...
            console.log("đầu vào của tải file lên ", this.productFile);
    ...
    myUploader(event) {
        if (event?.files[0]) {
            this._contractTemplateService.uploadFileGetUrl(event?.files[0], "product-legal-records").subscribe(
                (response) => {
                    console.log({
                        response,
                    });
    ...
    viewFile(fileUrl) {
        ...
        if(!fileUrl){
            this.messageError("Không có file hồ sơ", 3000)
        }else{
            if(this.utils.isPdfFile(fileUrl)){
                this.modalDialogPDF = true;
            } else {
                this.messageError("Hệ thống hiện tại chỉ hỗ trợ xem file PDF", 3000)
            }
        }
    }
}
