...
@Component({
  selector: 'app-distribution-price-list',
  templateUrl: './distribution-price-list.component.html',
  styleUrls: ['./distribution-price-list.component.scss']
})
export class DistributionPriceListComponent extends CrudComponentBase {
  titleFile: any;
  ...
  ref.onClose.subscribe((dataCallBack) => {
    ...
    this._distributionService.deletePrice(this.distributionId).subscribe((response) => {
      if (
  ...
  setPage(pageInfo?: any) {
    ...
    this._distributionService.getAllPrice(this.page, this.distributionId).subscribe((res) => {
  ...
  edit(bondSecondPrice) {
      ...
      console.log({ dataCallBack });
			if (dataCallBack?.accept) {
			this._distributionService.updatePrice(bondSecondPrice).subscribe((response) => {
			  if (
				this.handleResponseInterceptor(
				  response,
  ...
  myUploader(event) {
    ...
    if (this.fileExcel) {
      this.isLoading = true;
      this._distributionService.importPriceFromExcel({ file: this.fileExcel }, this.distributionId).subscribe(
        (response) => {
