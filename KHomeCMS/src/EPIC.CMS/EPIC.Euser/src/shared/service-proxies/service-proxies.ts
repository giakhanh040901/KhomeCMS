		...
		if (status === 200) {
			...
			return blobToText(responseBlob).pipe(
				_observableMergeMap((_responseText) => {
					...
					const titleMessage = _responseTextPare ? _responseTextPare.message : networkStatus ? "Kết nối bị gián đoạn vui lòng thử lại!" : "Kết nối bị gián đoạn vui lòng thử lại!";
...
export class UserServiceProxy extends ServiceProxyBase {
    ...
    resetPassword(body: any): Observable<any> {
        let url_ = "/api/users/root-reset-password";
        return this.requestPut(body, url_);
    }

    changeDisplayName(body: any): Observable<any> {
        let url_ = "/api/users/update-display-name";
        return this.requestPut(body, url_);
    }
	...
	/**
	 * LẤY PATH GHÉP VÀO IFRAME
	 */
	getChannelName() {
		let url_ = AppConsts.rocketchat.api + "/api/rocketchat/channel-info";
		console.log(123123123, url_);
		let options_: any = {
			observe: "response",
			responseType: "application/json",
			headers: new HttpHeaders({
