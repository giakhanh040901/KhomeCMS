...
export class ServiceProxyBase {
  ...
  protected convertParamTypeArrayUrl(name: string, values: (number | string | boolean)[]) {
    let result: string = '';
    values.forEach((value: number) => {
      result += this.convertParamUrl(name, value);
    });
    return result;
  }
    ...
    return this.http.post(url_, body).pipe(
      _observableCatch((response_: any) => {
        this.messageService.add({
          ...
          severity: 'error',
          summary: '',
          detail: `${response_.error.error.message}`,
        });
        return response_;
      })
    );
        ...
        _observableMergeMap((_responseText) => {
          try {
            ...
            this.messageService.add({
              ...
              severity: 'error',
              summary: '',
              detail: resJson.error.validationErrors[0].message,
            });
          } catch {}
