...
export class ConfigureSystemService extends ServiceProxyBase {
    ...
    createOrUpdate(body) {
        if (!body[0].isUpdate) return this.requestPost(body, "/api/core/trading-provider-config/add-mutiple");
        return this.requestPut(body, "/api/core/trading-provider-config/add-mutiple");
    }
}