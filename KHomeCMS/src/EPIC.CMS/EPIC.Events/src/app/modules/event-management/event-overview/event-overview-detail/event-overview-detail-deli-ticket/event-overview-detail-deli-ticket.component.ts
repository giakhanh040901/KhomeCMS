...
import { EventOverviewDetailDeliTicketModel } from '@shared/interface/event-management/event-overview/EventOverviewDetailDeliTicket.model';
...
@Component({
  selector: 'event-overview-detail-deli-ticket',
  templateUrl: './event-overview-detail-deli-ticket.component.html',
  styleUrls: ['./event-overview-detail-deli-ticket.component.scss'],
})
export class EventOverviewDetailDeliTicketComponent extends CrudComponentBase {
  public headerColumns: IHeaderColumn[] = [];
  public dataSource: EventOverviewDetailDeliTicketModel[] = [];
  ...
  private genListAction() {
    this.listAction = this.dataSource.map((data: EventOverviewDetailDeliTicketModel, index: number) => {
      const actions: IActionTable[] = [];
      
      if(this.isGranted(this.PermissionEventConst.TongQuanSuKien_ChiTiet_MauGiaoNhanVe_CapNhat)) { 
        actions.push({
          data: data,
          label: 'Chỉnh sửa',
          icon: 'pi pi-pencil',
      ...
      if(this.isGranted(this.PermissionEventConst.TongQuanSuKien_ChiTiet_MauGiaoNhanVe_XemThuMau)) { 
        actions.push({
          data: data,
          label: 'Xem thử mẫu',
      ...
      if(this.isGranted(this.PermissionEventConst.TongQuanSuKien_ChiTiet_MauGiaoNhanVe_TaiMauVe)) { 
        actions.push({
          data: data,
          label: 'Tải mẫu vé',
          icon: 'pi pi-download',
       ...
       if(this.isGranted(this.PermissionEventConst.TongQuanSuKien_ChiTiet_MauGiaoNhanVe_DoiTrangThai)) { 
  ...
  public detail(data: EventOverviewDetailDeliTicketModel) {
    if (data.id) {
      const dataSource = { ...data, label: 'giao nhận vé' };
      this.dialogService.open(CrudEventDetailTemplateComponent, {
        header: 'Xem chi tiết mẫu giao nhận vé',
        width: '600px',
        baseZIndex: 10000,
        data: {
  ...
  public edit(data: EventOverviewDetailDeliTicketModel) {
    if (data.id) {
      const dataSource = {
        ...data,
        label: 'vé',
        apiCrud: this.eventOverviewService.createOrEditDeliTicketOfEvent.bind(this.eventOverviewService),
      };
      const ref = this.dialogService.open(CrudEventDetailTemplateComponent, {
        header: 'Chỉnh sửa mẫu giao nhận vé',
        width: '600px',
        baseZIndex: 10000,
        data: {
          dataSource: dataSource,
  ...
  public create(event: any) {
    if (event) {
      if (this.dataSource.some((e: EventOverviewDetailDeliTicketModel) => e.status === EventOverview.KICH_HOAT)) {
        const refConfirm = this.dialogService.open(FormNotificationComponent, {
          ...
          baseZIndex: 10000,
          data: {
            title: 'Mẫu giao nhận vé hiện tại sẽ bị Hủy kích hoạt khi thêm mới mẫu giao nhận vé khác.',
        ...
        refConfirm.onClose.subscribe((dataCallBack) => {
          if (dataCallBack?.accept) {
            const ref = this.dialogService.open(CrudEventDetailTemplateComponent, {
              header: 'Thêm mới mẫu giao nhận vé',
              width: '600px',
              baseZIndex: 10000,
              data: {
                dataSource: {
  ...
  public deactivate(data: EventOverviewDetailDeliTicketModel) {
    if (data) {
      ...
      ref.onClose.subscribe((dataCallBack) => {
        if (dataCallBack?.accept) {
          this.eventOverviewService.changeStatusDeliTicketOfEvent(data.id).subscribe(
            (response) => {
            ...
            (err) => {
              this.messageError(`Không hủy kích hoạt được mẫu giao nhận vé`);
            }
          );
        }
      });
  ...
  public activate(data: EventOverviewDetailDeliTicketModel) {
    if (data) {
      const ref = this.dialogService.open(FormNotificationComponent, {
        header: 'Thông báo',
        width: '600px',
  ...
  public view(data: EventOverviewDetailDeliTicketModel) {
    if (data) {
      this.spinnerService.isShow = true;
      this.eventOverviewService.viewFileDeliTicketPDF(data.id).subscribe(
        (res: any) => {
  ...
  public download(data: EventOverviewDetailDeliTicketModel) {
    if (data) {
      this.eventOverviewService.downloadFileDeliTicketWord(data.id).subscribe();
    }
  }
  ...
  public setPage() {
    ...
    this.eventOverviewService.findAllDeliTicketOfEvent().subscribe(
      (res) => {
            ...
            this.dataSource = res.data.items.map(
              (item: any) =>
                ({
                  ...
                  status: item.status,
                } as EventOverviewDetailDeliTicketModel)
            );
            this.genListAction();
          }
        }
