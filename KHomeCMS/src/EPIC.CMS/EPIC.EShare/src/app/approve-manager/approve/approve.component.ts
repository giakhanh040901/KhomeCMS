...
import { FormCompanyShareInfoApproveComponent } from "./form-company-share-info-approve/form-company-share-info-approve.component";
...
export class ApproveComponent extends CrudComponentBase implements OnDestroy {
	...
	genListAction(data = []) {
		this.listAction = data.map(dataCompanyShare => {
			const actions = [
				{
					data: dataCompanyShare,
			...
			if (dataCompanyShare.status == ApproveConst.TRINH_DUYET && ((this.isGranted([this.PermissionCompanyShareConst.CompanyShareQLPD_PDLTP_PheDuyetOrHuy]) && this.checkApprovePage == 4)
			|| (this.isGranted([this.PermissionCompanyShareConst.CompanyShareQLPD_PDBTKH_PheDuyetOrHuy]) && this.checkApprovePage == 5))) {
				actions.push({
					data: dataCompanyShare,
					label: 'Phê duyệt',
					icon: 'pi pi-arrow-up',
					command: ($event) => {
						this.approveCompanyShare($event.item.data);
					}
				});
			}
	...
	approveCompanyShare(companyShareData) {
		console.log("company-shareDataâ",companyShareData);
		...
		ref.onClose.subscribe((dataCallBack) => {
			...
			if (dataCallBack?.accept) {
				...
				if (this.dataType == 5) {
					if (dataCallBack.checkApprove == true) {
						this._companyShareSecondaryService.approve(dataCallBack.data).subscribe((response) => {
							if (this.handleResponseInterceptor(response, "Phê duyệt thành công!")) {
								this.routeSubcribe = this.routeActive.params.subscribe((params) => {
									if (params.dataType) {
										this.setPage({ page: this.offset }, params.dataType);
						...
						this._companyShareSecondaryService.cancel(dataCallBack.data).subscribe((response) => {
							if (this.handleResponseInterceptor(response, "Huỷ duyệt thành công!")) {
								this.routeSubcribe = this.routeActive.params.subscribe((params) => {
									if (params.dataType) {
										this.setPage({ page: this.offset }, params.dataType);
	...
	detail() {
		switch (this.approve.dataType) {
			...
			case this.ApproveConst.STATUS_BUSINESS_CUSTOMER:
				...
				break;
			case this.ApproveConst.STATUS_PRO_COMPANY_SHARE_INFO:
				this.router.navigate(["/company-share-manager/company-share-info/detail/" + this.cryptEncode(this.approve.referId)]);
				break;
			case this.ApproveConst.STATUS_PRO_COMPANY_SHARE_SECONDARY:
				this.router.navigate(["/company-share-manager/company-share-secondary/update/" + this.cryptEncode(this.approve.referId)]);
				break;
			case this.ApproveConst.STATUS_PRO_COMPANY_SHARE_PRIMARY:
				this.router.navigate(["/company-share-manager/company-share-primary/detail/" + this.cryptEncode(this.approve.referId)]);
				break;
			default:
				break;
		}
