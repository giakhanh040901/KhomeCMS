...
export class DeliveryContractComponent extends CrudComponentBase {
  ...
  handleData(rows) {
    ...
    for (let row of rows) {
      row.kyHan = row?.companySharePolicyDetail?.shortName;
  ...
  genListAction(data = []) {
    this.listAction = data.map(blockadeLiberation => {
      ...
      if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_GiaoNhanHopDong_ThongTinChiTiet])){
        actions.push({
      ...
      if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_GiaoNhanHopDong_XuatHopDong])){
        actions.push({
          data: blockadeLiberation,
          label: 'Xuất hợp đồng',
          icon: 'pi pi-download',
      ...
      if (blockadeLiberation.deliveryStatus == this.DeliveryContractConst.CHO_XU_LY && this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_GiaoNhanHopDong_DoiTrangThai])) {
        actions.push({
          data: blockadeLiberation,
          label: 'Đang giao',
          icon: 'pi pi-send',
  ...
  exportContractReceive(data) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.isLoading = true;
        this._deliveryContractService.exportContractReceive(data.orderId, data.companyShareSecondaryId, data.tradingProviderId, data.source).subscribe((response) => {
  ...
  setPage(pageInfo?: any) {
    ...
    this.isLoading = true;
    this._orderService.getAllDeliveryContract(this.page, this.deliveryStatus, this.source).subscribe((res) => {
      this.isLoading = false;
