...
export class ContractBlockageComponent extends CrudComponentBase {
  ...
  genListAction(data = []) {
    this.listAction = data.map(blockadeLiberation => {
      ...
      if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_PTGT_XemChiTiet])){
        actions.push({
      ...
      if (blockadeLiberation.status == this.BlockageLiberationConst.PHONG_TOA && this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_PTGT_GiaiToaHopDong])) {
        actions.push({
          data: blockadeLiberation,
          label: 'Giải tỏa HĐ',
          icon: 'pi pi-key',
