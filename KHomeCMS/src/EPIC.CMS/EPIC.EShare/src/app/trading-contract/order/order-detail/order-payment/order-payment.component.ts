...
export class OrderPaymentComponent extends CrudComponentBase {
  ...
  genListAction(data = []) {
    this.listAction = data.map(orderItem => {
        ...
        if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_ChiTietThanhToan])){
          actions.push({
            data: orderItem,
        ...
        if (orderItem.status == this.OrderPaymentConst.PAYMENT_TEMP) {
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_CapNhat])){
            actions.push({
              data: orderItem,
              label: 'Sửa',
              icon: 'pi pi-pencil',
          ...
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_PheDuyet])){
            actions.push({
              data: orderItem,
              label: 'Phê duyệt',
              icon: 'pi pi-check',
          ...
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_Xoa])){
            actions.push({
              data: orderItem,
              label: 'Xóa',
              icon: 'pi pi-trash',
        ...
        if (orderItem.status == this.OrderPaymentConst.PAYMENT_SUCCESS) {
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_HuyPheDuyet])){
            actions.push({
              data: orderItem,
              label: 'Hủy phê duyệt',
              icon: 'pi pi-times',
          ...
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShareHDPP_SoLenh_TTCT_TTThanhToan_GuiThongBao])){
            actions.push({
              data: orderItem,
              label: 'Gửi thông báo',
              icon: 'pi pi-send',
  ...
  save() {
    this.submitted = true;
    //
    this.setTimeZoneList(this.fieldDates, this.orderPayment);
    console.log({ orderPayment: this.orderPayment });
    //
    if (this.orderPayment.orderPaymentId) {
      this._orderPaymentService.update(this.orderPayment).subscribe((response) => {
        //
        ...
        if (this.handleResponseInterceptor(response, 'Cập nhật thành công')) {
          ...
          this.callTriggerFiledError(response, this.fieldErrors);
          this.resetTimeZoneList(this.fieldDates, this.orderPayment);
          this.submitted = false;
        }
      }, () => {
        this.submitted = false;
      ...
      this._orderPaymentService.create(this.orderPayment).subscribe(
        (response) => {
