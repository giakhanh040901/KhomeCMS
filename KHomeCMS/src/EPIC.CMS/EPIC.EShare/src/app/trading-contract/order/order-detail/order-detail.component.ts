...
import { CompanyShareInfoServiceProxy, CompanySharePrimaryServiceProxy, CompanyShareSecondaryServiceProxy } from '@shared/service-proxies/company-share-manager-service';
...
export class OrderDetailComponent extends CrudComponentBase {
    ...
    fieldUpdates = {
        totalValue: {
            'isEdit': false,
            'apiPath': '/api/company-share/order/update/total-value?',
            'name': 'totalValue',
        },
        policy: {
            'isEdit': false,
            'apiPath': '/api/company-share/order/update/update-policy-detail?',
            'name': 'policyDetailId',
            'messageRequired': 'Vui lòng chọn kỳ hạn!',
        },
        referralCode: {
            'isEdit': false,
            'apiPath': '/api/company-share/order/update/referral-code?',
            'name': 'saleReferralCode',
            'messageRequired': 'Vui lòng chọn mã giới thiệu!',
        },
        bankAccount: {
            'isEdit': false,
            'apiPath': '/api/company-share/order/update/bank-account?',
    ...
    init() {
        ...
        this._orderService.get(this.orderId).subscribe((resOrder) => {
            if (this.handleResponseInterceptor(resOrder, '')) {
                ...
                this._orderService.getPriceDate(resOrder.data.secondaryId, resOrder.data.paymentFullDate ?? new Date()).subscribe((resPrice) => {
                    if (this.handleResponseInterceptor(resPrice, '')) {
                        ...
                        this.companyShareInfos.push({
                            ...
                            secondaryId: resOrder?.data?.secondaryId,
                        });
                        //
                        this.policies = this.companyShareSecondary?.policies ?? [];
                        this.policyInfo = this.policies.find(item => item.policyId == this.orderDetail.policyId);

                        this.policyDetails = this.policyInfo?.details ?? [];
                        this.policyDetail = this.policyDetails.find(item => item.policyDetailId == this.orderDetail?.policyDetailId);
    ...
    setDueDate(policyDetailId) {
        this.policyDetail = this.policyDetails.find(item => item.policyDetailId == policyDetailId);
        this.profit = this.policyDetail.profit;
    ...
    changeEdit() {
        ...
        if (this.isEdit) {
            this.setTimeZoneList(this.fieldDates, this.orderDetail);
            this._orderService.update(this.orderDetail).subscribe((response) => {
                if (this.handleResponseInterceptor(response, 'Cập nhật thành công')) {
                    this.setStatusEdit();
                } else {
                    this.resetTimeZoneList(this.fieldDates, this.orderDetail);
                }
            });
        } else {
            this.setStatusEdit();
