...
export class ProductCategoryComponent extends CrudComponentBase {
    ...
    this.productCategoryService.delete(this.productCategory.productCategoryId).subscribe(
      (response) => {
        ...
        this.messageService.add({
          severity: 'error',
          summary: '',
          detail: `Không xóa được Danh mục cổ phần ${this.productCategory.name}`,
          life: 3000,
        });
      }
    );
  ...
  header(): string {
    return this.productCategory?.productCategoryId > 0 ? 'Sửa Danh mục cổ phần' : 'Thêm Danh mục cổ phần'
  }
}
