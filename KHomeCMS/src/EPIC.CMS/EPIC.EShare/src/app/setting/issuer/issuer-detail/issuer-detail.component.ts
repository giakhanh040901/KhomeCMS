...
export class IssuerDetailComponent extends CrudComponentBase {
  ...
  changeEdit() {
    ...
    if (this.isEdit) {
      this.setTimeZoneList(this.fieldDates, this.issuerDetail);
      this.issuerService.update(this.issuerDetail).subscribe((response) => {
        ...
        if (this.handleResponseInterceptor(response, 'Cập nhật thành công')) {
          ...
          this.callTriggerFiledError(response, this.fieldErrors);
          this.resetTimeZoneList(this.fieldDates, this.issuerDetail);
        }
      });
    } else {
      this.isEdit = !this.isEdit;
