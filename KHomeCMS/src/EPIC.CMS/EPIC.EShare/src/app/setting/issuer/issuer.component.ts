...
export class IssuerComponent extends CrudComponentBase {
	...
	status = '';
	// statusSearch: any[] = [...ProductCompanyShareInfoConst.statusConst]
	// ProductCompanyShareInfoConst = ProductCompanyShareInfoConst;
	expandedRows = {};
	...
	genListAction(data = []) {
		this.listAction = data.map(issuerItem => {
			...
			if (this.isGranted([this.PermissionCompanyShareConst.CompanyShare_TCPH_ThongTinChiTiet])){
				actions.push({
					data: issuerItem,
			...
			if (this.isGranted([this.PermissionCompanyShareConst.CompanyShare_TCPH_Xoa])){
				actions.push({
					data: issuerItem,
					label: 'Xoá',
					icon: 'pi pi-trash',
