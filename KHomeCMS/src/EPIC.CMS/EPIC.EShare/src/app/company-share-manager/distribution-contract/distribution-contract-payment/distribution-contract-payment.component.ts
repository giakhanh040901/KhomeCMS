...
import { DistributionContractPaymentServiceProxy, DistributionContractServiceProxy } from '@shared/service-proxies/company-share-manager-service';
...
export class DistributionContractPaymentComponent extends CrudComponentBase {
  ...
  genListAction(data = []) {
    this.listAction = data.map(orderItem => {
        ...
        if (orderItem.status == this.DistributionContractConst.PAYMENT_TEMP){
          ...
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShare_HDPP_TTTT_PheDuyet])){
            actions.push({
              data: orderItem,
              label: 'Phê duyệt',
              icon: 'pi pi-check',
          ...
          if (this.isGranted([this.PermissionCompanyShareConst.CompanyShare_HDPP_TTTT_Xoa])){
            actions.push({
              data: orderItem,
              label: 'Xóa',
              icon: 'pi pi-trash',
        ...
        if ( (orderItem.status == this.DistributionContractConst.PAYMENT_SUCCESS) || (orderItem.status == this.DistributionContractConst.PAYMENT_CLOSE) 
            && this.isGranted([this.PermissionCompanyShareConst.CompanyShare_HDPP_TTTT_ChiTietThanhToan])){
          actions.push({
            data: orderItem,
        ...
        if (orderItem.status == this.DistributionContractConst.PAYMENT_SUCCESS && this.isGranted([this.PermissionCompanyShareConst.CompanyShare_HDPP_TTTT_HuyPheDuyet])) {
          actions.push(
          {
            data: orderItem,
            label: 'Hủy phê duyệt',
  ...
  save() {
    this.submitted = true;
    //
    this.setTimeZoneList(this.fieldDates, this.distributionContractPayment);
    console.log({ distributionContractPayment: this.distributionContractPayment });
    //
    if (this.distributionContractPayment.paymentId) {
      this._distributionContractPaymentService.update(this.distributionContractPayment).subscribe((response) => {
        //
        if (this.handleResponseInterceptor(response, 'Cập nhật thành công')) {
          ...
          this.resetTimeZoneList(this.fieldDates, this.distributionContractPayment);
          this.submitted = false;
        }
      }, () => {
        this.submitted = false;
      ...
      this._distributionContractPaymentService.create(this.distributionContractPayment).subscribe(
        (response) => {
