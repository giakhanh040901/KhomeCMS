...
import { ReceiveContractTemplateServiceProxy } from "@shared/service-proxies/company-share-manager-service";
...
export class ReceiveContractTemplateComponent extends CrudComponentBase implements OnInit {
	...
	@Input() companyShareSecondaryId: number;
	uploadedFiles: any[] = [];
	classifyId: number;
  ...
  rowsCheck: any;
	...
	genListAction(data = []) {
		this.listAction = data.map(contractTemplate => {
			...
			if (contractTemplate?.id != null && this.isGranted([this.PermissionCompanyShareConst.CompanyShare_BTKH_TTCT_MauGiaoNhanHD_CapNhat])) {
				actions.push({
					data: contractTemplate,
					label: 'Sửa',
					icon: 'pi pi-pencil',
	...
	setPage(pageInfo?: any) {
		...
		this._contractTemplateService.getAll(this.page, {classifyId: this.classifyId, companyShareSecondaryId: this.companyShareSecondaryId}).subscribe(
			(res) => {
				this.isLoading = false;
				if (this.handleResponseInterceptor(res, "")) 
        {
	...
	save() {
		this.contractTemplate.companyShareSecondaryId = +this.companyShareSecondaryId;
		//
		this.submitted = true;
