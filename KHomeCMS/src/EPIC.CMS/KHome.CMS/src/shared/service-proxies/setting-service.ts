import { AppConsts, TradingProviderConst } from '@shared/AppConsts';
import {
...
@Injectable()
export class GeneralContractorServiceProxy extends ServiceProxyBase {
    constructor(
...
export class TradingProviderServiceProxy extends ServiceProxyBase {
    ...
    updateStatus(body): Observable<any> {
        let url_ = `/api/core/trading-provider/change-status?`;
        url_ += this.convertParamUrl("tradingProviderId", body.tradingProviderId);
        url_ += this.convertParamUrl("status", (body.status === TradingProviderConst.KICH_HOAT) ? TradingProviderConst.HUY_KICH_HOAT : TradingProviderConst.KICH_HOAT);
    ...
    getTaxCode(taxCode :String): Observable<any> {
    ...
    getAllTradingProvider(page: Page, status: number): Observable<any> {
    ...
    getAllNoPagingAndPermission(): Observable<any> {
        return this.requestGet('/api/core/trading-provider/find-no-permission?pageSize=-1');
    }
