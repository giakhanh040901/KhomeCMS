...
import { AppConsts, ContractTypeConst, PermissionCoreConst, StatusBondInfoFileConst } from '@shared/AppConsts';
import { CrudComponentBase } from '@shared/crud-component-base';
import { ProductBondInfoFileServiceProxy } from '@shared/service-proxies/bond-manager-service';
...
export class BusiCusApproveLicenseFileComponent extends CrudComponentBase {
  ...
  myUploader(event) {
      if (event?.files[0]) {
          this._contractTemplateService.uploadFileGetUrl(event?.files[0], "business-customer-approve/license-file").subscribe(
              (response) => {
                  console.log({
                      response,
                  });
