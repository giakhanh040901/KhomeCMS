	...
	ngOnInit(): void {
		...
		this._userService.getAllPermission().subscribe(
			(res) => {
				...
				if (
					this.isGranted([
						PermissionRealStateConst.RealState_BaoCao_KinhDoanh_TQBangHangDuAn,
					])
				) {
					this.reports.push({
				...
				if (
					this.isGranted([
						PermissionRealStateConst.RealState_BaoCao_KinhDoanh_TH_TienVeDA,
					])
				) {
					this.reports.push({
				...
				if (
					this.isGranted([
						PermissionRealStateConst.RealState_BaoCao_KinhDoanh_TH_CacKhoanGD,
					])
				) {
					this.reports.push({
