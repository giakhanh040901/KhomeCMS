...
import { AppConsts, NotificationTemplateConst, TabView, TypeFormatDateConst, NotifyManagerConst, SaleConst, SearchConst } from '@shared/AppConsts';
...
export class NotificationDetailComponent extends CrudComponentBase implements OnInit {
  ...
  filteredsecondLevelOptions(event?) {
    ...
    const eventHandlers = {
      ...
      [NotificationTemplateConst.EVENT]: this.getEvent,
      default: () => {
  ...
  getEvent() {
    this._notificationExtendService.getEvent().subscribe((res) => {
      this.isLoading = false;
      if (this.handleResponseInterceptor(res, '')) {
        this.secondLevelOptions = [];
        res.data.forEach(item => {
  ...
  onSubmit() {
    if (this.postForm.valid) {
      if (this.currentNotification) {
        ...
        let appointment = this.formatDate(this.postForm.value.appointment, TypeFormatDateConst.YMDTHmsZ)
