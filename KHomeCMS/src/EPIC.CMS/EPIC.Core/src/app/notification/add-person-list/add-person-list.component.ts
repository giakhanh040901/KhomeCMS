...
export class AddPersonListComponent extends CrudComponentBase implements OnInit {
    ...
    setPersonList(pageInfo?: any) {
        ...
        this._investorService.getInvestorListNotify(this.personListPage, this.dataFilter).subscribe((res) => {
    ...
    addPeopleToSendingList() {
        console.log('listPersonNoti', this.selectedCustomers);
        ...
        console.log('listPersonNoti', customers, this.uniqCustomers, this.exitedUserErrorList, this.exitedPersonCodeList);
    ...
    selectedAll(statusCheckedAll) {
        if (statusCheckedAll) {
            this.isLoadingPersonList = true;
            this.personListPage.keyword = this.keyword;
            this._investorService.getInvestorListNotify(this.personListPage, this.dataFilter, true).subscribe((res) => {
                if (this.handleResponseInterceptor(res, "")) {
                    if (Array.isArray(res.data?.items)) {
                        let i = 0;
