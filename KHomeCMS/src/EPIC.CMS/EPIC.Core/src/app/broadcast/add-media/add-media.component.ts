...
export class AddMediaComponent extends CrudComponentBase implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.config.data.inputData) {
      ...
      this.mainLink = this.sanitizer.bypassSecurityTrustResourceUrl(this.inputData.mainImg);
      this.postForm = this.fb.group({
        ...
        imageType: [this.inputData.imageType, []],
        mainImg: [this.inputData.mainImg, []],
      });
  ...
  funcImage() {
    this.postForm.patchValue({
      mainImg: this.getEmbedURL(this.postForm?.value?.mainImg),
    })
    this.mainImg = this.postForm?.value?.mainImg;
    this.mainLink = this.sanitizer.bypassSecurityTrustResourceUrl(this.postForm.value.mainImg);
  }
  ...
  getEmbedURL(url: string): string {
    if(!url.includes('https://www.youtube.com')) {
      this.messageService.add({
        ...
        detail: 'Vui lòng nhập lại',
        life: 3000,
      });
    }
    let splitArray = url.split('=');
    let videoId = splitArray[splitArray.length - 1];
