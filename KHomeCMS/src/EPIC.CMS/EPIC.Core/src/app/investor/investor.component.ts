...
import { ProductBondInfoConst, ProductBondDetailConst, InvestorConst, SearchConst, FormNotificationConst, TypeFormatDateConst } from "@shared/AppConsts";
...
export class InvestorComponent extends CrudComponentBase {
	...
	showData(rows) {
		for (let row of rows) {
			...
			row.email = row.email;
			row.profStartDate = this.formatDate(row.profStartDate,TypeFormatDateConst.DMY);
			row.profDueDate = this.formatDate(row.profDueDate,TypeFormatDateConst.DMY);
