...
export class NotificationService extends ServiceProxyBase {
	...
	getAll(page: Page,status: string,sortData?: any[]): Observable<any> {
		...
		if (status) {
			url_ += this.convertParamUrl("statusSent", status);
		}
		if(sortData?.length){
	...
	getAllNotificationTemplate(page: Page,status?: any,sortData?: any[]): Observable<any> {
	...
	getPersonList(notificationId, page: Page, dataFilter, filters, isAll = false): Observable<any> {
