...
import { ContractTemplateServiceProxy, DistributionContractFileServiceProxy, GuaranteeAssetServiceProxy } from "@shared/service-proxies/bond-manager-service";
...
@Component({
    selector: 'app-product-bond-info-collateral',
    templateUrl: './product-bond-info-collateral.component.html',
    styleUrls: ['./product-bond-info-collateral.component.scss'],
    providers: [ConfirmationService, MessageService]
})
export class ProductBondInfoCollateralComponent extends CrudComponentBase {
    ...
    genListAction(data = []) {
        this.listAction = data.map(bondInfoCollateralItem => {
            ...
            if (this.isGranted([this.PermissionBondConst.Bond_LTP_TSDB_Sua])){
				actions.push({
            ...
            if (this.isGranted([this.PermissionBondConst.Bond_LTP_TSDB_Xoa])){
				actions.push({
                    data: bondInfoCollateralItem,
                    label: 'Xóa',
                    icon: 'pi pi-trash',
        ...
        this.listFilesAction = data.map(bondInfoCollateralItem => {
            let filesAction = [];
            if (this.isGranted([this.PermissionBondConst.Bond_LTP_TSDB_TaiXuong])){
    ...
    create() {
        ...
        this.collateral.productBondId = +this.productBondId;
    ...
    setPage(pageInfo?: any) {
        ...
        this._guaranteeAssetService.getAll(this.page, this.productBondId).subscribe(
            (res) => {
