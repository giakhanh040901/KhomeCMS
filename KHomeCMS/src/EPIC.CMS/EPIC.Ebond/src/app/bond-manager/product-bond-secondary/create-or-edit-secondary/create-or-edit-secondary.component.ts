...
import { ProductBondInterestConst, ProductPolicyConst } from "@shared/AppConsts";
...
export class CreateOrEditSecondaryComponent extends CrudComponentBase {
	...
	@Input() header: string = '';
	@Input() productBondSecondary: any = {};
	@Input() listPrimary = [];
	...
	save() {
		this.onSave.emit(this.productBondSecondary);
	}
	...
	findPrimary(id) {
		const found = this.listPrimary.find(p => p.bondPrimaryId === id);
		return found || {};
	}
	...
	validForm(): boolean {
		const validRequired = this.productBondSecondary.openCellDate
							&& this.productBondSecondary.closeCellDate
							&& this.productBondSecondary.businessCustomerBankAccId;
