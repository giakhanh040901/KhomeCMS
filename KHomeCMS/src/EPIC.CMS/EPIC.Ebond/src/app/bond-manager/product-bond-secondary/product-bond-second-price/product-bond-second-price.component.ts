...
@Component({
  selector: 'app-product-bond-second-price',
  templateUrl: './product-bond-second-price.component.html',
  styleUrls: ['./product-bond-second-price.component.scss']
})
export class ProductBondSecondPriceComponent extends CrudComponentBase {
  titleFile: any;

  constructor(
    injector: Injector,
    private routeActive: ActivatedRoute,
    private _productBondSecondaryServiceProxy: ProductBondSecondaryServiceProxy,
  ...
  ref.onClose.subscribe((dataCallBack) => {
    ...
    this._productBondSecondaryServiceProxy.deleteSecondPrice(this.bondSecondaryId).subscribe((response) => {
      if (
  ...
  confirmDelete() {
    ...
    this._productBondSecondaryServiceProxy.deleteSecondPrice(this.bondSecondaryId).subscribe(
      (response) => {
  ...
  setPage(pageInfo?: any) {
    ...
    this._productBondSecondaryServiceProxy.getAllSecondPrice(this.page, this.bondSecondaryId).subscribe((res) => {
  ...
  // edit(bondSecondPrice) {
  //   this.confirmationService.confirm({
  //     message: `Bạn có chắc chắn sửa giá ở ngày ${moment(bondSecondPrice.priceDate).format('DD/MM/YYYY')}?`,
  //     header: 'Cảnh báo',
  //     icon: 'pi pi-exclamation-triangle',
  //     acceptLabel: 'Đồng ý',
  //     rejectLabel: 'Hủy',
  //     accept: () => {
  //       this._productBondSecondaryServiceProxy.updateSecondPrice(bondSecondPrice).subscribe(
  //         (response) => {
  ...
  edit(bondSecondPrice) {
      ...
      console.log({ dataCallBack });
			if (dataCallBack?.accept) {
			this._productBondSecondaryServiceProxy.updateSecondPrice(bondSecondPrice).subscribe((response) => {
			  if (
				this.handleResponseInterceptor(
				  response,
  ...
  myUploader(event) {
    ...
    if (this.fileExcel) {
      this.isLoading = true;
      this._productBondSecondaryServiceProxy.importPriceFromExcel({ file: this.fileExcel }, this.bondSecondaryId).subscribe(
        (response) => {
