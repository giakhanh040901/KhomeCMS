...
import { ProductBondInterestConst, ProductBondSecondaryConst, ProductPolicyConst, SearchConst, YesNoConst } from "@shared/AppConsts";
...
@Component({
	selector: "app-product-bond-secondary",
	templateUrl: "./product-bond-secondary.component.html",
	styleUrls: ["./product-bond-secondary.component.scss"],
	providers: [DialogService],
})
export class ProductBondSecondaryComponent extends CrudComponentBase {
	constructor(
		injector: Injector,
	...
	statusSearch: any[] = [
		{
		  ...
		  ...ProductBondSecondaryConst.statusList
	];
	...
	ngOnInit() {
		...
		this.onChangeIntestType({ value: ProductBondInterestConst.INTEREST_TYPES.DINH_KY });

		this.cols = [
			{ field: 'bondCode', header: 'Mã TP', width: '5rem', class: 'justify-content-start', cutText: 'b-cut-text-5', isPin: true },
			{ field: 'bondName', header: 'Tên trái phiếu', width: '17rem', class: 'b-cut-text-20', pTooltip: 'Hạn mức đầu tư còn lại', tooltipPosition: 'top', isPin: true },
		...
		// this._selectedColumns = this.getLocalStorage('bondSecondary') ?? this.cols;
	}
	...
	setLocalStorage(data) {
		return localStorage.setItem('bondSecondary', JSON.stringify(data));
	}
	...
	setPage(pageInfo?: any) {
		...
		// 	this.isClose = 'N';
		// }
		forkJoin([this._secondaryService.getAllBondPrimary(), this._secondaryService.getAll(this.page, this.status, this.isClose), this._policyService.getAllTemporaryNoPaging()]).subscribe(
			([resBondPrimary, resBondSecondary, resTempPolicy]) => {
				this.isLoading = false;
				// set type isCheck sang kiểu boolean
				if (this.handleResponseInterceptor(resBondSecondary, '')){
					if (resBondSecondary.data?.items) {
						this.rows = resBondSecondary.data.items.map(row => {
				...
				this.listPrimary = resBondPrimary?.data ?? [];
				if(this.rows?.length) { 
					this.genListAction(this.rows);
					this.showData(this.rows);
				}
				
				this.page.totalItems = resBondSecondary?.data?.totalItems;
				this.search.listPolicy = [...resTempPolicy?.data?.items];

				console.log({ productBondPrimay: resBondPrimary, rows: resBondSecondary, resBondPrimary: resBondPrimary });
			},
			(err) => {
				this.isLoading = false;
	...
	genListAction(data = []) {
		this.listAction = data.map(secondaryItem => {
			...
			if (this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_ThongTinChiTiet])){
				actions.push({
					data: secondaryItem,
			...
			if (secondaryItem.status == this.ProductSecondaryConst.STATUS.NHAP && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_ThongTinChiTiet])) {
				actions.push({
					data: secondaryItem,
					label: 'Trình duyệt',
					icon: 'pi pi-arrow-up',
			...
			// if (secondaryItem.status == this.ProductSecondaryConst.STATUS.TRINH_DUYET && this.isGranted(this.PermissionBondConst.BondMenuQLTP_BTKH_TrinhDuyet)) {
			// 	actions.push({
			// 		data: secondaryItem,
			// 		label: 'Phê duyệt',
			// 		icon: 'pi pi-check',
			...
			if (secondaryItem.status == this.ProductSecondaryConst.STATUS.HOAT_DONG && secondaryItem.isCheck == false && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_EpicXacMinh])) {
				actions.push({
					data: secondaryItem,
			...
			if (secondaryItem.isClose == YesNoConst.STATUS_NO && secondaryItem.status == this.ProductSecondaryConst.STATUS.HOAT_DONG && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_DongTam])) {
				actions.push({
					data: secondaryItem,
					label: 'Đóng tạm',
					icon: 'pi pi-lock',
			...
			if (secondaryItem.isClose == YesNoConst.STATUS_YES && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_DongTam])) {
				actions.push({
					data: secondaryItem,
					label: 'Mở',
					icon: 'pi pi-unlock',
			...
			if (secondaryItem.isShowApp == YesNoConst.STATUS_NO && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_BatTatShowApp])) {
				actions.push({
					data: secondaryItem,
					label: 'Bật show app',
					icon: 'pi pi-eject',
			...
			if (secondaryItem.isShowApp == YesNoConst.STATUS_YES && this.isGranted([this.PermissionBondConst.BondMenuQLTP_BTKH_BatTatShowApp])) {
				actions.push({
					data: secondaryItem,
					label: 'Tắt show app',
	...
	create() {
		// this.expandAll();
		this.productBondSecondary = { ...BASE.SECODARY, policies: [] };
	...
	view(productBondSecondary) {
		this._router.navigate(["bond-manager/product-bond-secondary/update/" + this.cryptEncode(productBondSecondary?.bondSecondaryId)]);
	}
	 ...
	 */
	save(bondSecondary) {
		this.productBondSecondary = { ...bondSecondary };
		this.submitted = true;
		console.log({ Secondary: this.productBondSecondary });
		//
		let body = this.formatCalendar(this.fieldDates, {...this.productBondSecondary});
		this._secondaryService.create(body).subscribe(
			(response) => {
	...
	validForm(): boolean {
		const validRequired = this.productBondSecondary?.bondPrimaryId;
	...
	confirmDelete(type) {
		...
		if (type == POLICY) {
			if (this.policy) {
				let index = this.productBondSecondary.policies.findIndex((item) => item.fakeId == this.policy.fakeId);
				this.productBondSecondary.policies.splice(index, 1);
			}
		} else if (type == POLICY_DETAIL) {
			if (this.policyDetail) {
				for (let policy of this.productBondSecondary.policies) {
	...
	savePolicy() {
		if (this.policy?.fakeId) {
			...
			let index = this.productBondSecondary.policies.findIndex((item) => item.fakeId == this.policy.fakeId);
			this.productBondSecondary.policies[index] = { ...this.policy };
			console.log({ BondSecondary: this.productBondSecondary });
		} else {
			this.policy.action = this.ProductPolicyConst.ACTION_CREATE;
			this.policy.fakeId = new Date().getTime();
			if (!this.productBondSecondary.policies) {
				this.productBondSecondary.policies = [];
			}
			this.productBondSecondary.policies.push(this.policy);
		}
	...
	savePolicyDetail() {
		...
		if (this.policyDetail.fakeId) {
			this.policy.action = this.ProductPolicyConst.ACTION_UPDATE;
			let index = this.productBondSecondary.policies[policyIndex].details.findIndex((item) => item.fakeId == this.policyDetail.fakeId);
			this.productBondSecondary.policies[policyIndex].details[index] = { ...this.policyDetail };
		} else {
			this.policy.action = this.ProductPolicyConst.ACTION_CREATE;
			this.policyDetail.fakeId = new Date().getTime();
			if (!this.productBondSecondary.policies[policyIndex].details) {
				this.productBondSecondary.policies[policyIndex].details = [];
			}
			this.productBondSecondary.policies[policyIndex].details.push(this.policyDetail);
			this.expandedRows[this.productBondSecondary.policies[policyIndex].fakeId] = true;
			console.log(this.productBondSecondary.policies[policyIndex].details);
		}

		console.log({ productBondSecondary: this.productBondSecondary });
	...
	// DUYỆT / BỎ DUYỆT
	approve(productBondSecondary) {
		const ref = this.dialogService.open(
			FormApproveComponent,
			this.getConfigDialogServiceRAC("Phê duyệt", productBondSecondary?.bondSecondaryId)
		);
		//
		ref.onClose.subscribe((dataCallBack) => {
			console.log('dataCallBack', dataCallBack);
	...
	onChangeIntestType($event) {
		const { value } = $event;
		if (ProductBondInterestConst.isDinhKy(value)) {
			this.policyDetail.interestPeriodType = ProductBondInterestConst.interestPeriodTypes[0].code;
		} else {
