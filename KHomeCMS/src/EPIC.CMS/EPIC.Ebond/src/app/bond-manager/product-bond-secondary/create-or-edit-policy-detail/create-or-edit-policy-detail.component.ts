...
export class CreateOrEditPolicyDetailComponent implements OnInit {
    ...
    validForm(): boolean {
        const validRequired = this.policyDetail?.periodQuantity 
							...
							&& ((this.policyDetail?.interestType == ProductBondPolicyDetailTemplateConst.INTEREST_RATE_TYPE_PERIODIC && this.policyDetail?.interestPeriodQuantity && this.policyDetail?.interestPeriodType) || this.policyDetail?.interestType == ProductBondPolicyDetailTemplateConst.INTEREST_RATE_TYPE_PERIOD_END);
