...
import { BusinessTypeConst, ContractTemplateConst, ContractTypeConst, FormNotificationConst, ProductBondSecondaryConst, SearchConst } from "@shared/AppConsts";
...
export class ContractTemplateComponent extends CrudComponentBase implements OnInit {
	...
	typesSearch: any[] = [
		...
		...ProductBondSecondaryConst.types
	];
	...
	genListAction(data = []) {
		this.listAction = data.map(contractTemplate => {
			...
			if (this.isGranted([this.PermissionBondConst.Bond_BTKH_TTCT_MauHopDong_CapNhat])) {
                actions.push(
                    {
			...
			if (this.isGranted([this.PermissionBondConst.Bond_BTKH_TTCT_MauHopDong_Xoa])) {
                actions.push(
                    {
						data: contractTemplate,
						label: 'Xoá',
			...
			if (contractTemplate.status == this.ContractTemplateConst.status.DEACTIVE && this.isGranted([this.PermissionBondConst.Bond_BTKH_TTCT_MauHopDong_KichHoatOrHuy])) {
				actions.push({
					data: contractTemplate,
					label: 'Kích hoạt',
					icon: 'pi pi-check',
			...
			if (contractTemplate.status == this.ContractTemplateConst.status.ACTIVE && this.isGranted([this.PermissionBondConst.Bond_BTKH_TTCT_MauHopDong_KichHoatOrHuy])) {
				actions.push({
					data: contractTemplate,
					label: 'Huỷ kích hoạt',
					icon: 'pi pi-times',
	...
	setPage(pageInfo?: any) {
		...
		this._contractTemplateService.getAll(this.page, {classifyId: this.classifyId, bondSecondaryId: this.bondSecondaryId, type: this.type}).subscribe(
			(res) => {
				this.isLoading = false;
				if (this.handleResponseInterceptor(res, "")) {
	...
	validForm(): boolean {
		return this.contractTemplate?.contractTempUrl?.trim() !== "" && this.contractTemplate?.code?.trim() && this.contractTemplate?.name?.trim()
				&& this.contractTemplate?.type?.trim();
	}
