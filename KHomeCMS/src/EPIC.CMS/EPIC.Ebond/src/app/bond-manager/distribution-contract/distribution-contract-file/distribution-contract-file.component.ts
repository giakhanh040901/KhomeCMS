	...
	genListAction(data = []) {
		this.listAction = data.map(orderItem => {
			...
			if (orderItem.status == this.DistributionContractConst.PENDING) {
				if (this.isGranted([this.PermissionBondConst.Bond_HDPP_DMHSKHK_PheDuyet])){
					actions.push({
						data: orderItem,
						label: 'Phê duyệt',
						icon: 'pi pi-check',
			...
			if (orderItem.status == this.DistributionContractConst.SUCCESS && this.isGranted([this.PermissionBondConst.Bond_HDPP_DMHSKHK_HuyPheDuyet])) {
			  actions.push(
			  {
				data: orderItem,
				label: 'Hủy duyệt',
