...
export class CreateOrderComponent extends CrudComponentBase {
  ...
  ngOnInit() {
    ...
    this.items = [
      ...
      },
      {
        ...
        routerLink: '/trading-contract/order/create/filter-product-bond'
      },
      {
  ...
  clickDropdown(row) {
    // this.productBondInfo = { ...row };
    // this.actionsDisplay = this.actions.filter(action => action.statusActive.includes(row.status) && action.permission);
  }

  detail() {
    // this.router.navigate(['/bond-manager/product-bond-info/detail/' + this.productBondInfo.productBondId]);
  }
  ...
  setPage(pageInfo?: any) {
    ...
    this._productBondInfoService.getAll(this.page, this.status).subscribe((res) => {
