...
export class OrderContractComponent extends CrudComponentBase {
    ...
    cancelSign(){
        this.confirmationService.confirm({
            message: 'Bạn có chắc chắn hủy ký điện tử hồ sơ?',
            header: 'Hủy ký điện tử hồ sơ',
            acceptLabel: "Đồng ý",
			rejectLabel: "Hủy",
            icon: 'pi pi-question-circle',
            accept: () => {
                this._orderServiceProxy.cancelSign(this.orderDetail.orderId).subscribe((res) => {
                    if (this.handleResponseInterceptor(res, 'Hủy ký điện tử thành công')) {
                        this.setPage();
                    };
                });
            },
    ...
    myUploader(event) {
        if (event?.files[0]) {
            this._contractTemplateServiceProxy.uploadFileGetUrl(event?.files[0], "bond").subscribe(
                (response) => {
                    console.log({
                        response,
                    });
