...
export class ContractActiveComponent extends CrudComponentBase {
  ...
  ngOnInit() {
    ...
    this.cols = [
      ...
      { field: 'bondPolicyName', header: 'Sản phẩm', width: '11rem'},
      { field: 'bondPolicyDetailName', header: 'Kỳ hạn', width: '8rem'},
  ...
  genListAction(data = []) {
    this.listAction = data.map(orderItem => {
      ...
      if (this.isGranted([this.PermissionBondConst.BondHDPP_SoLenh_TTCT_TTC])){
        actions.push({
          data: orderItem,
      ...
      //                                                
      if (orderItem?.status == OrderConst.DANG_DAU_TU && this.isGranted([this.PermissionBondConst.BondHDPP_SoLenh_TTCT_HSKHDangKy_GuiThongBao])) {
        actions.push({
          data: orderItem,
          label: 'Gửi thông báo',
					icon: 'pi pi-envelope',
      ...
      //                                                
      if (orderItem?.status == OrderConst.DANG_DAU_TU && this.isGranted([this.PermissionBondConst.BondHDPP_HopDong_YeuCauTaiTuc]) && !orderItem?.isRenewalsRequest) {
				actions.push({
					data: orderItem,
      ...
      //
      if (orderItem?.status == OrderConst.DANG_DAU_TU && this.isGranted([this.PermissionBondConst.BondHDPP_HopDong_PhongToaHopDong])) {
				actions.push({
					data: orderItem,
					label: 'Phong tỏa HĐ',
          icon: 'pi pi-ban',
  ...
  saveBlockade() {
    this.confirmationService.confirm({
      ...
      accept: () => {
        let body = this.formatCalendar(['blockadeDate'], {...this.blockageLiberation});
        this._blockageLiberationService.blockadeContractActive(body).subscribe((response) => {
