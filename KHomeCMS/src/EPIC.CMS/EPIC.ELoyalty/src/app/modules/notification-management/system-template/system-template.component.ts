...
export class SystemTemplateComponent extends CrudComponentBase implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.notificationService.getSystemNotification('loyalty').subscribe(data => { 
  ...
  saveSetting() {
    ...
    this.notificationService.createOrUpdateSystemNotification(settings, 'loyalty').subscribe(data => {
      return this.messageService.add({
        severity: 'success',
