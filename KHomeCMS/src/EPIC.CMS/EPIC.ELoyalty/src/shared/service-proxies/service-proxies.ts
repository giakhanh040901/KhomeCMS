...
import { Observable, throwError as _observableThrow, of as _observableOf, throwError, timer } from 'rxjs';
import { Injectable, Inject, Optional, InjectionToken } from '@angular/core';
import { HttpClient, HttpHeaders, HttpParams, HttpResponse, HttpResponseBase } from '@angular/common/http';

import * as moment from 'moment';
import { API_BASE_URL, blobToText, ServiceProxyBase, throwException } from './service-proxies-base';
import { AppConsts, PermissionLoyaltyConst } from '@shared/AppConsts';
    ...
    if (status === 200) {
      ...
      return blobToText(responseBlob).pipe(
        _observableMergeMap((_responseText) => {
          ...
          const titleMessage = _responseTextPare
            ? _responseTextPare.message
            : networkStatus
...
export class UserServiceProxy extends ServiceProxyBase {
  ...
  getAllPermission(): Observable<any> {
    let url_ = `/api/users/get-permission?permissionInWeb=${PermissionLoyaltyConst.WEB_LOYALTY}`;
    return this.requestGet(url_);
  }
  ...
  // LÂY THÔNG TIN ĐĂNG NHẬP VÀO HỆ THỐNG
  getPrivacyInfo() {
      return this.requestGet('/api/users/get-privacy-info');
  }
}
